def hexxor(a, b):     # xor two strings of different lengths
    c = hex(a)
    d = hex(b)
    if len(c) > len(d):
       return hex(int(c[:len(d)], 16) ^ int(d, 16)) 
    else:
       return hex(int(d[:len(c)], 16) ^ int(c, 16))

ct1  = 
ct2  = 0x234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f
ct3  = 0x32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb
ct4  = 
ct5  = 0x3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070
ct6  = 
ct7  = 
ct8  = 
ct9  = 
ct10 = 0x466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83

tgt  = 0x32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904

ct   = 0x66396e89c9dbd8cc9874352acd6395102eafce78aa7fed28a07f6bc98d29c50b69b0339a19f8aa401a9c6d708f80c066c763fef0123148cdd8e802d05ba98777335daefcecd59c433a6b268b60bf4ef03c9a611098bb3e9a3161edc7b804a3

ctarr = [ct1,ct2,ct3,ct4,ct5,ct6,ct7,ct8,ct9,ct10]

marr = []
for i in ctarr:
    new = []
    for j in ctarr:
        if i != j:
            new.append(hexxor(i, j))
    marr.append(new)

spacearr = []
for i in range(len(marr)):
    count = []
    space = []
    for j in range(0, 400):
        count.append(0)
    for m in marr[i]:
        for n in range(2, len(m), 2):
            z = int(m[n:n+2], 16) ^ 32
            if (z >= 0x41 and z <= 0x7A) or z == 32 or z == 33 or z == 44 or  z == 46 or z == 63:
                # print(bytearray.fromhex(m[n:n+2]).decode())
                count[n] += 1
           # else:
    #            print(m[n:n+2])
    for k in range(len(count)):
         if count[k] == 9:
             #print("%d %d" % (k, i))
             space.append(k)
    spacearr.append(space)

for i in range(len(spacearr)):
    for k in range(len(marr[i])):
        for j in spacearr[i]:
            #print("CT " + str(j/2) + " in line " + str(k) + "is:")
            s = hex(ctarr[i])
            y = int(s[j:j+2], 16) ^ 32
            #print(hex(y))
            # z = int((marr[i][k])[j:j+2], 16) ^ 32
            
ans = hexxor(ct, tgt)
print(ans)
res = []
for i in ctarr:
    res.append(hexxor(i, ct))
res.append(hexxor(ct, tgt))
for i in res:
    for n in range(2, len(i), 2):
            z = int(i[n:n+2], 16)
            y = int(hex(ct)[n:n+2], 16)
            if y != 0 and z >= 0x20 and z <= 0x7A:
                print(bytearray.fromhex(i[n:n+2]).decode(), end ="")
            else:
                print("_", end ="")
    print()

